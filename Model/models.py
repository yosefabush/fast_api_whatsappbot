...
class ConversationSession(Base):
    ...
    def all_validation(self, db, step, answer):
        match step:
            ...
            case 2:
                ...
                # user = User.log_in_user(user=self.get_converstion_step('1'), password=answer)
            ...
            case 3:
                ...
                if answer not in list(choises.keys()):
                    return False
            case 4:
    ...
    def validate_and_set_answer(self, db, step, response):
        ...
        if self.all_validation(db, step, response):
            if step == 3:
                chises_dict = {a.name: a.id for a in db.query(Items).all()}
    ...
    def get_chossies(self, db):
        ...
        choices = {a.name: a.id for a in db.query(Items).all()}
